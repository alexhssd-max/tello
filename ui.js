...
function renderBubbles(containerId, arr, highlightIndex = -1, highlightClass = 'found') {
  ...
  if (!container) return; // Si el contenedor no existe en el HTML, evita que se rompa el código
  container.innerHTML = '';
...
$('btn-confirm').addEventListener('click', () => {
  ...
  show('sec-search');
  show('sec-sort');
  hide('sec-report'); // Se mantiene oculto hasta que se presione Ordenar

  // Resetear resultados previos de forma segura
  hide('search-result');
  hide('sort-results');
  hide('add-more-panel');
...
$('btn-add-confirm').addEventListener('click', () => {
  ...
  // LIMPIEZA CORREGIDA: Ya no busca el id obsoleto 'add-nums'
  $('add-n').value = '';
  $('dynamic-add-inputs').innerHTML = ''; 
  hide('add-more-panel');

  // Resetear resultados anteriores ya que el array cambió
  hide('search-result');
  hide('sort-results');
  hide('sec-report');
  lastSearch = null;
...
/* ═══════════════ SECCIÓN 04 — Ordenamiento e Inyección Automática al Reporte ═══════════════ */
$('btn-sort').addEventListener('click', () => {
  ...
  // 1. Renderizado en paneles normales de ordenamiento
  ...
  // 2. Renderizado automático dentro de tu NUEVO Reporte HTML
  ...
  // 3. Mostrar bloques y hacer scroll suave
  show('sort-results');
  show('sec-report'); // Muestra la sección 05 automáticamente
