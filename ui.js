...
const clearErr = id => { const el = $(id); if (el) el.textContent = ''; };

/* ═══════════════ Render burbujas (Resultados) ═══════════════ */
...
/* ═══════════════ SECCIÓN 04 — Ordenamiento e Inyección al Reporte ═══════════════ */
$('btn-sort').addEventListener('click', () => {
  ...
  $('asc-comparisons').textContent = `comparaciones realizadas: ${asc.comparisons}`;

  renderBubbles('desc-bubbles', desc.sorted);
  $('desc-comparisons').textContent = `comparaciones realizadas: ${desc.comparisons}`;

  // Renderizado en tu nueva estructura HTML del reporte (Sección 05)
  renderBubbles('report-original', currentArray);

  renderBubbles('report-asc', asc.sorted);
  $('report-asc-comp').textContent = `${asc.comparisons} comparaciones`;

  renderBubbles('report-desc', desc.sorted);
  $('report-desc-comp').textContent = `${desc.comparisons} comparaciones`;
...
<svg viewBox="0 0 760 1660" xmlns="http://www.w3.org/2000/svg"
  ...
  <text x="380" y="40" text-anchor="middle" fill="#e8eaf6" font-family="'Syne',sans-serif" font-size="15" font-weight="700" letter-spacing="2">DIAGRAMA DE FLUJO — BUBBLE SORT</text>
  <text x="380" y="58" text-anchor="middle" fill="#6b7280" font-size="11">Ascendente y Descendente</text>
  <rect x="30" y="70" width="130" height="24" rx="4" fill="#1a2040"/>
  <text x="95" y="87" text-anchor="middle" fill="#4f7cff" font-size="11" letter-spacing="1">ASCENDENTE ↑</text>
  ...
  <text x="660" y="87" text-anchor="middle" fill="#7c5cfc" font-size="11" letter-spacing="1">DESCENDENTE ↓</text>
  ...
  <g transform="translate(0, 40)">
    ...
    <text x="380" y="1124" text-anchor="middle" fill="#4f7cff" font-size="11" letter-spacing="2">BÚSQUEDA LINEAL (array original)</text>
  ...
  const targetContainer = $('flowchart-svg');
  if (targetContainer) targetContainer.innerHTML = svg;
}

/* ═══════════════ CORRECCIÓN DE BURBUJAS DINÁMICAS (INIT) ═══════════════ */
  ...
  function updateDynamicInputs(countId, containerId) {
    ...
    if (count > currentCount) {
      for (let i = currentCount; i < count; i++) {
        // CORRECCIÓN CLAVE: El input se crea ahora con las clases exactas 
        // para heredar los estilos redondos de tu CSS (.bubble + .bubble-input)
        const inp = document.createElement('input');
        inp.type = 'number';
        inp.className = 'bubble bubble-input'; 
        inp.placeholder = '0';
